...
*/
static int64_t string_to_bytes(char *size_str)
{
       int64_t size = 0;
       char range;
       int rc;
...
static void option_parse_token(char ** argv, int * flag_parsed_next, int * requiredArgsSeen, int * error, int * print_help, option_help * args){
  char * txt = argv[0];
  char * arg = strstr(txt, "=");
  ...
  *flag_parsed_next = 0;
  ...
  for(size_t flag_index = 0; flag_index < strlen(txt); ++flag_index){
...
int poption_parse(int argc, char ** argv, option_help * args){
  int error = 0;
  ...
  for(i=1; i < argc; i++){
    ...
    option_parse_token(& argv[i], & flag_parsed_next, & requiredArgsSeen, & error, & printhelp, args);
    if (flag_parsed_next){
      i++;
    }
    if(error){
